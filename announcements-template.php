	...
	<div id="main-content" class="<?php echo esc_attr($page_column_class['main_class']); ?>">	
		<div id="primary" class="site-content blog-list-style">
			...
			<?php	
				...
				$posts = new WP_Query( array('post_type'=>'announcements', 'paged'=>$paged) );
				if( $posts->have_posts() ):
					echo '<div class="list-posts herehere">';
					while( $posts->have_posts() ) : $posts->the_post();
						get_template_part( 'announcements-content', get_post_format() ); 
					endwhile;
					echo '</div>';
					
					wp_reset_postdata();
				else:
					echo '<div class="alert alert-error">'.esc_html__('Sorry. There are no careers to display', 'boxshop').'</div>';
				endif;
